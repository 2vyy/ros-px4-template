...
| B53 | Determinism stack: auto-rosbag every sim run → `logs/run_<ts>.bag`; `just replay <bag>`; seedable Gazebo + PX4 RNG; `tools/fault_inject.py` for GPS dropout / wind / motor failure | idea | Biggest missing affordance for "rapid ROS dev" — turns one-shot scenarios into deterministic regression assets |
...
3. **B53** — determinism + replay + fault injection. Auto-rosbag every run, `just replay`, seedable physics, `tools/fault_inject.py`. Turns "I can demo arm+takeoff" into "I can iterate on edge-case mission logic in minutes" — the actual promise of rapid ROS dev.
