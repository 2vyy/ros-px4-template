...
def test_check_token_validates_subcommands(tmp_path: Path) -> None:
    root = _justfile(tmp_path, "log", "cap", "sim", "wait")
    assert check_docs.check_token("just log tail", "just", root) is True
    assert check_docs.check_token("just log since", "just", root) is True
    ...
    assert check_docs.check_token("just sim start --gui", "just", root) is True
    assert check_docs.check_token("just sim frobnicate", "just", root) is False
    assert check_docs.check_token("just wait run --timeout 120", "just", root) is True
